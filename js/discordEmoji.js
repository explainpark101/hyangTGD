...
let aritcle = document.querySelector("div#article-content");
let comments = document.querySelectorAll("div.reply-content");

emote_names.forEach((name)=>{
    aritcle.innerHTML = article.innerHTML.replace(`:${name}:`, `<img src="${emoticons[name]}">`);
    comments.forEach((comment)=>{
        comment.innerHTML = comment.innerHTML.replace(`:${name}:`, `<img src="${emoticons[name]}">`);
    })
});